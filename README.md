...
Some dealer sites block server-side page extraction. Version `1.0.12` adds an optional fallback panel on the audit form:

- Homepage fallback content
- VDP / inventory fallback content
- Service page fallback content
- Additional audit notes

When pasted content is supplied, the backend treats it as user-provided source material and uses it as the primary source for that page if scraping is blocked or incomplete. This lets you generate an audit from:

- copied visible page text
- page notes from a saved PDF
- notes from screenshots
- manually captured price, fee, service, brand, and CTA details

The app still attempts normal extraction, then combines any successful scrape with the pasted fallback content.
...
Test `/api/ping`. It should return JSON and show version `1.0.12`.
...
## v1.0.12 PDF pagination refinements
