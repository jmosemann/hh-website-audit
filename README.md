...
## v1.0.16 PDF pagination refinements
...
## v1.0.16 PDF fallback note

If scraping is blocked, upload saved PDFs for the homepage, VDP, and service page. The app now extracts PDF text before sending the audit request, stores the PDF in Netlify Blobs, and passes the extracted text into the audit prompt. This prevents blocked-scrape reports when uploaded PDFs contain usable page text.

For best results, create PDFs with the browser's Print > Save as PDF option rather than scanned images. Image-only PDFs may not contain extractable text; paste page notes into the fallback text boxes when that happens.


## v1.0.16 note

This version fixes a PDF fallback handoff issue from v1.0.15.

In v1.0.15, uploaded PDF files could show extracted text in the browser but still not reach the audit worker because the top-level `homepageFiles`, `vdpFiles`, and `serviceFiles` arrays were not included when `create-audit` normalized fallback content. The result could be a blocked-access audit even after uploading readable PDFs.

v1.0.16 now passes uploaded PDF references through `create-audit`, stores them in the job payload, and makes the audit prompt treat fallback/uploaded source text as the primary audit source when scraping is blocked.

After deploying, check `/api/ping` and confirm it returns `"version": "1.0.16"`.
