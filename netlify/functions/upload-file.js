...
import { extractPdfTextFromDataUrl } from './lib/pdf-text.js';
...
export default async function uploadFile(req) {
  ...
  try {
    ...
    let text = typeof body.text === 'string' ? body.text.slice(0, 24000) : '';
    const dataUrl = typeof body.dataUrl === 'string' ? body.dataUrl : '';

    if (!text && dataUrl && dataUrl.startsWith('data:application/pdf;base64,')) {
      text = extractPdfTextFromDataUrl(dataUrl, 24000);
    }
