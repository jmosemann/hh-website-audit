...
function App() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Could not create audit.');
      setAudit(data.audit);
...
async function readJsonResponse(response, endpointName) {
  ...
  try {
    ...
    if (looksLikeHtml) {
      throw new Error(
        `${endpointName} returned HTML instead of JSON. Netlify Functions are probably not running or the function route is not deployed. ` +
