        ...
        <form onSubmit={submit} className="form-card">
          ...
          <details className="fallback-panel">
            ...
            <p>
              ...
              text into these fields. PDFs are sent as source attachments to the audit generator; text files are read and added
...
function FileUpload({ label, files = [], onChange }) {
  ...
  return (
    <label className="field file-field">
      ...
      />
      <small className="file-hint">Upload saved page PDFs or text/HTML files. PDFs are stored first so the audit request stays small. Keep each file under 2.5 MB.</small>
      {localError && <small className="file-error">{localError}</small>}
      {files.length > 0 && (
        <ul className="file-list">
          {files.map((file) => (
            <li key={`${file.name}-${file.size}`}>
              {file.name} <span>{formatBytes(file.size)}{file.blobKey ? ' · uploaded' : ''}</span>
            </li>
          ))}
        </ul>
      )}
