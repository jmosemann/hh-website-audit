...
function FileUpload({ label, files = [], onChange }) {
  ...
  return (
    <label className="field file-field">
      ...
      />
      <small className="file-hint">Upload saved page PDFs or text/HTML files. Keep each file under 4 MB.</small>
...
async function readAuditFiles(files) {
  ...
  for (const file of selected) {
    if (file.size > maxBytes) {
      throw new Error(`${file.name} is too large. Keep each uploaded file under 4 MB.`);
    }
    ...
    if (type === 'application/pdf') {
      ...
      prepared.push({
        ...
        mimeType: type,
        size: file.size,
        text: (await file.text()).slice(0, 24000)
      });
    }
  }
